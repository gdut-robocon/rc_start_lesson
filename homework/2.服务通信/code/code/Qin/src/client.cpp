...
/*
 客户端：提交两个整数，并处理响应的整数
   ...
   解决：
   在ROS中内置相关函数，这些函数可以让客户端启动后挂起，等待服务器启动
...
{    //优化实现，获取命令中的参数
    ...
    ros::NodeHandle nh;
    // 4.创建一个客户端对象
    ros::ServiceClient client = nh.serviceClient<ros_tutorials_service::AddInts>("add_tw0_ints");
    // 5.提交请求并处理响应
  ...
  // client.waitForExistence();
    ros::service::waitForService("add_tw0_ints");
